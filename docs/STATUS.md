...
- A Beam checkpoint contains the Lean server's accepted environment. Elaborators can distinguish
  server execution from batch execution, so a successful checkpoint is not a guarantee that its
  artifact is identical to a fresh `lake build` artifact. Use checkpoints during the development
  loop, then run the clean batch-validation sequence in
  [SYNC_AND_DIAGNOSTICS.md](SYNC_AND_DIAGNOSTICS.md#development-checkpoints-and-authoritative-builds)
  before merging, releasing, or treating CI as authoritative.
