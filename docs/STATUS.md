...
- successful `lean_init_workspace` responses advertise projected MCP capability names such as
  `beam_version`, `lean_run_at`, `lean_sync`, `lean_save`, `lean_hover`, `lean_goals_prev`, and
  `lean_goals_after`
...
Lean position and range operations are version-bound across the broker, MCP, and wrapper surfaces.
Clients must first update or sync the file, then pass the returned broker document version.
`update` is the cheap version-producing step; `sync` is for clients that also need
diagnostics/readiness. Broker-detected stale-version failures use `contentModified` with
