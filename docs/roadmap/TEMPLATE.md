# CARD-0000 Short Title

Status: open | candidate-0.2.0 | deferred | close-candidate | resolved
Kind: bug | ux | diagnostics | performance | feature | upstream-api | cleanup
Priority: low | medium | high | critical
Origin: local | LIRIS | upstream Lean backlog
Last reviewed: YYYY-MM-DD

## Summary

One paragraph stating the agent-visible or maintainer-visible problem.

## Impact

- What a user, agent, or maintainer tried to do.
- What Beam or Lean returned.
- Why the result is confusing, unsafe, slow, or expensive to maintain.

## Beam Decision

The current roadmap decision and release target, if any.

## Expected Behavior

The smallest behavior or API change that would make the workflow reliable.

## Evidence

Links to sanitized public evidence, tests, docs, or upstream discussions.

## Current Workaround

The operational rule to follow until the card is resolved.
