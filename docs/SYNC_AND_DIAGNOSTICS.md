...
Position- or range-bound operations reject missing or stale versions; clients must obtain the
version from `lean-beam sync`, broker `sync_file`, or MCP `lean_sync` before sending those
operations.
