---
name: lean-beam
description: Use this when an AI should work on an external Lean project through the installed `beam` wrapper, giving it direct efficient access to Lean's proof engine to avoid rebuilds through cheap speculative checks and zero-build module checkpoints.
---
...
The installer puts `beam` and `beam-lean-search` in `~/.local/bin`, stages the self-contained
...
This skill documents the supported Lean-facing `beam` workflow surface. Use the smallest command
family that fits the task.

Supported command families:

- bootstrap the Lean backend: `beam ensure lean`
...
- run shell-oriented search loops over the same handle APIs: `beam-lean-search`
...
- stale-state, `contentModified`, or rebuild trouble keeps appearing; inspect with `beam open-files`
  and `beam doctor lean`
...
- keeps one Beam daemon per project root and records it in `<root>/.beam/beam-daemon.json`
  - in sandboxed or read-only project trees, set `BEAM_CONTROL_DIR` to a writable directory; `beam` uses a per-root subdirectory there
...
- fails early on unsupported Lean toolchains; use `beam supported-toolchains lean` to inspect the allowlist
...
- if you want shorter shell commands for search loops: use `beam-lean-search`
- if bundle resolution or startup looks wrong: check `beam doctor lean` before guessing
...
- prefer `beam lean-run-at` before editing when feasible
- treat `beam lean-sync` as mandatory after every real Lean file edit before the next speculative probe
...
- prefer `beam lean-save` / `lean-close-save` over a full `lake build` when only one file needs checkpointing
...
- if daemon/save-state behavior looks wrong, inspect `beam open-files` and `beam doctor lean`
