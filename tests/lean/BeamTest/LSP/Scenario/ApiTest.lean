...
namespace BeamTest.LSP.Scenario.ApiTest
...
private def requireSingleGoalTarget (label : String) (result : Beam.LSP.RunAt.Result) : ScenarioM String := do
...
private def checkSyncRunAtInterleavings : ScenarioM Unit := do
  ...
  syncDoc doc
  let beforeBarrierResult : Beam.LSP.RunAt.Result ← awaitResponseAs beforeBarrier
...
end BeamTest.LSP.Scenario.ApiTest

def main := BeamTest.LSP.Scenario.ApiTest.main
