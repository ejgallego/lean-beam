...
namespace BeamTest.LSP.Scenario.MctsProofSearchTest
...
private def goalCountOf (label : String) (result : Beam.LSP.RunAt.Result) : ScenarioM Nat := do
  let some proofState := result.proofState?
    | throw <| IO.userError s!"{label}: expected proofState payload"
  pure proofState.goals.size

private def awaitResult (label : String) (req : ReqHandle) : ScenarioM Beam.LSP.RunAt.Result := do
...
private def disposeSolvedHandle? (doc : DocHandle) : Option Beam.LSP.RunAt.Handle → ScenarioM Unit
  | some handle =>
...
end BeamTest.LSP.Scenario.MctsProofSearchTest

def main := BeamTest.LSP.Scenario.MctsProofSearchTest.main
