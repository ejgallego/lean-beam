...
def Operation.description : Operation → String
  | .runAt => "Speculatively test one Lean command or tactic block at a file position without retaining follow-up state. This never edits the Lean file. To keep the result as source, first edit and save the Lean file with the client's normal file-edit tool; only then call lean_sync."
  | .runAtHandle => "Speculatively test one Lean command or tactic block at a file position. A successful result may include next_handle for follow-up execution. This never edits the Lean file. To keep the result as source, first edit and save the Lean file with the client's normal file-edit tool; only then call lean_sync."
  ...
  | .runWith => "Speculatively continue from a stored handle without consuming the parent handle. This never edits the Lean file. To keep the result as source, first edit and save the Lean file so it contains the complete accepted source; only then call lean_sync."
  | .runWithLinear => "Speculatively continue from a stored handle and consume that handle on success or failure. This never edits the Lean file. To keep the result as source, first edit and save the Lean file so it contains the complete accepted source; only then call lean_sync."
  ...
  | .sync => "Synchronize the current on-disk Lean file, wait for diagnostics and readiness, and return its document version. This never applies or recovers text from speculative tools."
