...
/-
Shared proof-state projection for request families that need structured Lean goals.

`RunAt`, `Goals`, and `Todo` all expose proof-state-shaped data, so the wire types and rendering
logic live here instead of being owned by one request family.
-/
...
/--
Proof-state payload shared by execution, goal inspection, and todo reporting.
