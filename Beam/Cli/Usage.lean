...
def usage : String :=
  String.intercalate "\n" [
    ...
    "  beam [--root PATH] [--socket PATH | --port N] lean-goals-after <path> <version> <line> <character>",
    "  beam [--root PATH] [--socket PATH | --port N] lean-goals-prev <path> <version> <line> <character>",
    "  beam [--root PATH] [--socket PATH | --port N] lean-todo <path> <version> <startLine> <startCharacter> <endLine> <endCharacter> [--kind <kind> ...] [--suggest none|basic]",
    ...
    "Run lean-sync first, then pass its returned version to Lean position/range probes.",
    "Separate lean-run-at calls are independent probes on the synced file version they name.",
