...
def usage : String :=
  String.intercalate "\n" [
    ...
    "Feedback is returned locally, not submitted; review non-confidential reports before posting",
    "because they may contain project context and caller-supplied payloads.",
    "Feedback input must be a JSON object with required string fields: title, summary,",
    "reproduction, expected, and actual.",
    "Set confidential=true for non-public workspaces; confidential cards omit project-derived",
    "debug context and attachments and must not be posted publicly.",
