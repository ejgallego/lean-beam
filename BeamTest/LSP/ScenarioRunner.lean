...
namespace BeamTest.LSP.ScenarioRunner
...
private structure FrontState where
  ...
  handles : Std.TreeMap String Beam.LSP.RunAt.Handle := {}
...
private def getStoredHandle (handleName : String) : FrontM Beam.LSP.RunAt.Handle := do
...
private def executeStep (scriptStep : ScriptStep) : FrontM Unit := do
  try
    ...
    | .awaitHandle handleName reqName =>
        ...
        let result : Beam.LSP.RunAt.Result ← BeamTest.LSP.Scenario.awaitResponseAs (α := Beam.LSP.RunAt.Result) (← getReqHandle reqName)
    ...
    | .releaseHandle docName handleName =>
        BeamTest.LSP.Scenario.releaseHandle (← getDocHandle docName) (← getStoredHandle handleName)
    | .cancelReq reqName =>
        BeamTest.LSP.Scenario.cancelReq (← getReqHandle reqName)
    | .awaitReq reqName =>
        discard <| BeamTest.LSP.Scenario.awaitReq (← getReqHandle reqName)
    | .expectResponse reqName expected =>
        BeamTest.LSP.Scenario.expectResponseContains (← getReqHandle reqName) expected
    | .expectError reqName expected =>
        BeamTest.LSP.Scenario.expectErrorContains (← getReqHandle reqName) expected
  catch e =>
...
partial def main (args : List String) : IO Unit := do
  ...
  let some (path : String) := args[0]?
    | throw <| IO.userError "usage: lake exe beam-lsp-scenario-test <file>"
  ...
  discard <| BeamTest.LSP.Scenario.run do
...
end BeamTest.LSP.ScenarioRunner

def main := BeamTest.LSP.ScenarioRunner.main
