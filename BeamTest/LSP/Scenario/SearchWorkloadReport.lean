...
namespace BeamTest.LSP.Scenario.SearchWorkloadReport
...
private inductive StepOutcome where
  ...
  | solved (extraHandle? : Option Beam.LSP.RunAt.Handle)
...
private def goalCountOf (label : String) (result : Beam.LSP.RunAt.Result) : ReportM Nat := do
...
private def awaitResult (label : String) (req : ReqHandle) : ReportM Beam.LSP.RunAt.Result := do
  let outcome : BeamTest.TestHarness.RequestOutcome ← liftScenario <| awaitReq req
...
private def releaseHandleTimed (doc : DocHandle) (handle : Beam.LSP.RunAt.Handle) : ReportM Unit :=
  timed .release <| liftScenario <| releaseHandle doc handle

private def runAtTimed (doc : DocHandle) (spec : SendRunAtSpec) : ReportM Beam.LSP.RunAt.Result :=
  timed .mint do
    let req ← liftScenario <| sendRunAt doc spec
    awaitResult "root mint" req

private def runWithTimed (kind : OpKind) (label : String) (doc : DocHandle) (handle : Beam.LSP.RunAt.Handle)
    (spec : RunWithSpec) : ReportM Beam.LSP.RunAt.Result :=
  timed kind do
...
private def disposeSolvedHandle? (doc : DocHandle) : Option Beam.LSP.RunAt.Handle → ReportM Unit
  | some handle =>
...
end BeamTest.LSP.Scenario.SearchWorkloadReport

def main := BeamTest.LSP.Scenario.SearchWorkloadReport.main
