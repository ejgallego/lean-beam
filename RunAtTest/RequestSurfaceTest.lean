...
private def requireTodoKindCount
    ...
    (kind : RunAt.TodoKind)
    (expected : Nat)
    (result : RunAt.TodoResult) : ScenarioM Unit := do
...
private def todoMessageContains (needle : String) (item : RunAt.TodoItem) : Bool :=
...
private def requireTodoKindWithMessage
    ...
    (kind : RunAt.TodoKind)
    (needle : String)
    (result : RunAt.TodoResult) : ScenarioM RunAt.TodoItem := do
...
private def checkComplexTodoFalsePositives (doc : DocHandle) : ScenarioM Unit := do
  let falsePositiveReq ← sendTodo doc {
    startLine := RunAtTest.TodoFixture.complexFalsePositiveStartLine
    startCharacter := 0
    endLine := RunAtTest.TodoFixture.complexFalsePositiveEndLine
    endCharacter := 0
    kinds? := some #[.sorry]
  }
  let falsePositiveTodos : RunAt.TodoResult ← awaitResponseAs falsePositiveReq
...
private def checkComplexTodoSemanticItems (doc : DocHandle) : ScenarioM Unit := do
  let semanticReq ← sendTodo doc {
    startLine := RunAtTest.TodoFixture.complexStartLine
    startCharacter := RunAtTest.TodoFixture.complexStartCharacter
    endLine := RunAtTest.TodoFixture.complexEndLine
    endCharacter := RunAtTest.TodoFixture.complexEndCharacter
    kinds? := some #[.sorry, .hole]
  }
  let semanticTodos : RunAt.TodoResult ← awaitResponseAs semanticReq
...
private def checkComplexTodoIncompleteProofs (doc : DocHandle) : ScenarioM Unit := do
  let incompleteReq ← sendTodo doc {
    startLine := RunAtTest.TodoFixture.complexIncompleteOneStartLine
    startCharacter := 0
    endLine := RunAtTest.TodoFixture.complexIncompleteOneEndLine
    endCharacter := 0
    kinds? := some #[.incompleteProof]
    suggest? := some .basic
  }
  let incompleteTodos : RunAt.TodoResult ← awaitResponseAs incompleteReq
  ...
  let branchReq ← sendTodo doc {
    startLine := RunAtTest.TodoFixture.complexBranchSkipStartLine
    startCharacter := 0
    endLine := RunAtTest.TodoFixture.complexBranchSkipEndLine
    endCharacter := 0
    kinds? := some #[.incompleteProof]
    suggest? := some .none
  }
  let branchTodos : RunAt.TodoResult ← awaitResponseAs branchReq
...
private def checkComplexTodoDiagnostics (doc : DocHandle) : ScenarioM Unit := do
  let warningReq ← sendTodo doc {
    startLine := RunAtTest.TodoFixture.complexSorryStartLine
    startCharacter := 0
    endLine := RunAtTest.TodoFixture.complexSorryEndLine
    endCharacter := 0
    kinds? := some #[.diagnostic]
  }
  let warningTodos : RunAt.TodoResult ← awaitResponseAs warningReq
  ...
  let diagnosticReq ← sendTodo doc {
    startLine := RunAtTest.TodoFixture.complexDiagnosticLine
    startCharacter := 0
    endLine := RunAtTest.TodoFixture.complexDiagnosticLine
    endCharacter := RunAtTest.TodoFixture.complexDiagnosticEndCharacter
    kinds? := some #[.diagnostic]
  }
...
private def checkComplexTodoCodeActions (doc : DocHandle) : ScenarioM Unit := do
  let codeActionReq ← sendTodo doc {
    ...
    startCharacter := RunAtTest.TodoFixture.complexCodeActionStartCharacter
    endLine := RunAtTest.TodoFixture.complexCodeActionLine
    endCharacter := RunAtTest.TodoFixture.complexCodeActionEndCharacter
    kinds? := some #[.codeAction]
  }
  let codeActionTodos : RunAt.TodoResult ← awaitResponseAs codeActionReq
...
private def checkComplexTodoRequest : ScenarioM Unit := do
  let doc ← openDoc RunAtTest.TodoFixture.complexRepoPath
  syncDoc doc
